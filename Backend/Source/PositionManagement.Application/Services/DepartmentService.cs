...
{
    /// <summary>
    /// Provides services for managing departments, including CRUD operations and querying departments with optional related data.
    /// </summary>
        ...
        IBaseRepository<Department, Guid> departmentRepository) : IDepartmentService
    {
        ...
        private async Task<bool> Validator(string name, Guid? excludeId = null)
        ...
        public async Task<List<Department>> GetAllAsync(bool includePositions = true)
        {
        ...
        {
            /* Check if a department with the same name already exists and
             * Throw an exception if a duplicate department is found. */

            if (await Validator(department.Name))
        ...
        public async Task DeleteAsync(Guid id)
        {
            ...
            var departmentDb = await GetByIdAsync(id);

            // Delete the department from the repository
            await _departmentRepository.DeleteAsync(departmentDb);
        }
    }
}
